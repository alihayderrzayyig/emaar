        ...
        <div class="row flex-row-reverse">
            ...
            <div class="admin-content col-9">
                <div class="container">
                    ...
                    {{-- فورم البحث --}}
                    <h1 class="text-center">المساعدات</h1>
                    @if ($gifts->count())
                        <form action="{{ route('admin.gift.index') }}" method="get" class="mb-4">
                            <div class="input-group flex-row-reverse mb-3">
                                <input type="text" name="search" class="form-control" placeholder="ادخل الرقم التسلسلي"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($gifts as $gift)
                                        <tr>
                                            ...
                                            <td>{{ $gift->id }}</td>
                                            <td><a
                                                    href="{{ route('admin.users.show', $gift->user_id) }}">{{ $gift->name }}</a>
                                            </td>
                                            <td>{{ $gift->phone }}</td>
                                            <td>
                                                @if ($gift->situation_id != null)
                                                    <a
                                                        href="{{ route('admin.situation.show', $gift->situation->id) }}">{{ $gift->situation->name }}</a>
                                                @endif
                                            </td>
                                            <td class="d-flex">
                                                <div class="ml-2 flex btn btn-info btn-table">
                                                    <button onclick="handlshow('{{ $gift->id }}')" type="button"
                                                        data-toggle="modal" data-target="#messageShow{{ $gift->id }}"><i
                                            ...
                                            @endif --}}
                                                ...
                                                <form action="{{ route('admin.gift.destroy', $gift->id) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <div class="ml-2 flex btn btn-danger btn-table">
                                                        <button onclick="return confirm('هل انت متاكد من عملية الحذف')" type="submit"><i class="far fa-trash-alt"></i></button>
                                        ...
                                        <!-- View Modal -->
                                        <div class="modal fade" id="messageShow{{ $gift->id }}" tabindex="-1"
                    ...
                    @else
                        <h5 class="text-center">لا يوجد مساعدات ليتم تقديمها</h5>
                    @endif

...
@php $active_sidebar = 'joinus'; @endphp
@section('content')
<section id="admin">
    <div class="row flex-row-reverse">
        ...
        <div class="admin-content col-9">
            <div class="container">
                ...
                <h2 class="text-center mb-3">طلبات الانظمام</h2>

                @if ($JoinUsmessages->count())
                    <div class="card">
                        <table dir="rtl" class="table table-striped" style="overflow-y:auto;">
                            ...
                            <tbody>
                                @foreach ($JoinUsmessages as $item)
                                    <tr>
                                        ...
                                        <td class="d-flex">
                                            ...
                                            <div class="flex btn btn-danger btn-table ml-2">
                                                <button onclick="handldelete2('/admin/admin-joinus/{{ $item->id }}')" data-toggle="modal" data-target="#deleteCategory"><i class="far fa-trash-alt"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                    <!-- View Modal -->
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <p class="m-0">
                                                        ...
                                                        @foreach ($governorates as $item2)
                                                            @if ($item2->id == $item->governorate)
                                                                {{ $item2->name }} -
                                                            @endif
                                                        @endforeach
                                                        @foreach ($districts as $item2)
                                                            @if ($item2->id == $item->district)
                                                                {{ $item2->name }} -
                                                            @endif
                                                        @endforeach
                                                        {{ $item->region }}
                                                ...
                                                <div class="flex btn btn-danger btn-table mx-auto btn-sm mb-2">
                                                    <button onclick="handldelete2('/admin/admin-joinus/{{ $item->id }}')" data-toggle="modal" data-target="#deleteCategory">حذف</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    {{-- نهاية مودل الحذف --}}
                @else
                    <h5 class="text-center mb-3">لا يوجد طلبات ليتم عرضها</h5>
                @endif
